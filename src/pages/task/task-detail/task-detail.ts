...
export class TaskDetailPage {
  ...
  getTaskImages() {
      let url = this.httpService.getUrl()+"/getImagesOfTask.do";
  ...
  getOneTaskImage(imageId) {
      let url = this.httpService.getUrl()+"/getOneImageOfTask.do"+"?id="+imageId+"&type=0";
  ...
  showRadio() {
    ...
    taskUpdateStatus(status) {
        ...
        let url = this.httpService.getUrl()+"/updateAlarmTaskOfMobile.do";
        let headers = new Headers({
            'Content-Type': 'application/x-www-form-urlencoded'
        });
